    ...
    >
      ...
      <div style={{ marginBottom: '8px' }}>
        ...
        >
          ...
          <option value="exclusive">Exclusive Choice</option>
        </select>
      </div>
...
// Legacy export for existing n-ary relation rendering (if used elsewhere)
export function NaryRelation({ relation, allNodes, zoom, canvasOffset, isSelected, onSelect }) {
  ...
  return (
    <g className="nary-relation">
      ...
      {relation.activities.map((activityName, index) => {
        const node = allNodes.find(n => n.name === activityName);
        if (!node) return null;
