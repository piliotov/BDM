...
export function NaryRelationEditMenu({
  ...
  // Handle constraint number change
  const handleChangeConstraintN = (e) => {
    ...
    setDiagram(diagram => ({
      ...
      relations: diagram.relations.map(r =>
        r.id === relation.id ? { ...r, n: clampedN } : r
      )
    }));
  };

  // Handle adding a node to the n-ary relation
  const handleAddNode = (nodeId) => {
    ...
    // Update constraint if it exceeds new maximum
  ...
  // Handle removing a node from the n-ary relation
  ...
  // Get available nodes that aren't already in the relation
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          >
            ...
            <option value="Ex_choice">Ex Choice</option>
          </select>
        </div>
        <button
