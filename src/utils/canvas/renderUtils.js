// Rendering utilities for ConDecCanvas
...
/**
 * Renders the multi-select bounding box
 * @param {Object} params - Parameters object
 * @param {Array} params.multiSelectedNodes - Array of selected nodes
 * @param {Object} params.multiSelectedElements - Object with nodes, relationPoints, naryDiamonds
 * @param {number} params.zoom - Zoom level
 * @param {Object} params.diagram - Current diagram (for real-time positions during drag)
 * @param {Object} params.multiDragStart - Multi-drag state (to detect active dragging)
 * @returns {JSX.Element|null} Bounding box element or null
 */
export function renderMultiSelectBoundingBox({ multiSelectedNodes, multiSelectedElements, zoom, diagram, multiDragStart }) {
  ...
  if (hasExtendedSelection) {
    ...
    // If we're dragging, use current diagram positions
...
/**
 * Renders the multi-select menu
 * @param {Object} params - Parameters object
 * @param {Array} params.multiSelectedNodes - Array of selected nodes
 * @param {Object} params.multiSelectedElements - Object with nodes, relationPoints, naryDiamonds
 * @param {Object} params.props - Props object with callbacks
 * @param {Object} params.diagram - Current diagram (for real-time positions during drag)
 * @param {Object} params.multiDragStart - Multi-drag state (to detect active dragging)
 * @returns {JSX.Element|null} Menu element or null
 */
export function renderMultiSelectMenu({ multiSelectedNodes, multiSelectedElements, props, diagram, multiDragStart }) {
  ...
  function handleDeleteAll() {
    if (hasExtendedSelection) {
      // Use extended delete handler for mixed selections that include n-ary diamonds
      if (typeof props.onDeleteMultiSelectedExtended === 'function') {
        ...
        // Fallback to regular delete for nodes only
        props.onDeleteMultiSelected(multiSelectedElements.nodes);
      }
    } else {
      // Use regular delete handler for node-only selections
...
/**
 * Renders hologram node preview
 * @param {Object} params - Parameters object
 * @param {string} params.mode - Current mode
 * @param {Object} params.hologramNodePosition - Hologram position
 * @param {Object} params.diagram - Diagram object
 * @param {number} params.zoom - Zoom level
 * @returns {JSX.Element|null} Hologram node element or null
 */
